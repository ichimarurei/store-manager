...
const buildUpdateData = async (params: any): Promise<Partial<SalesDocument>> => {
    const authorized = await buildAuthorPayload<SalesDocument>(params.operator, 'update', params?.author);
...
export const update = async (params: any): Promise<SalesDocument | null> => {
    ...
    try {
        if (!isEmpty(params?.products)) {
            ...
            saved = await salesSchema.findOneAndUpdate({ _id: params._id }, payload, { new: true, lean: true }).lean<SalesDocument>();
